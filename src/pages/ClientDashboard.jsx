        ...
        <div className="card p-5">
          ...
          : <div className="space-y-3">
              {milestones.map((m, i) => (
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 text-xs
                    ${m.completed ? "bg-brand-cyan text-brand-darker" : "border-2 border-white/20"}`}
                  >
                    {m.completed ? "✓" : ""}
                  </div>
                  <div>
            ...
            {todos.map((item) => (
              ...
              >
                <div
                  ...
                  ${item.done ? "bg-green-500/20 text-green-400 border border-green-500/30" : "border-2 border-white/20"}`}
                >
                  {item.done ? "✓" : ""}
                </div>
                <p
    ...
    >
            ...
            >
              <div
                className={`max-w-sm rounded-2xl px-4 py-3 ${
                  msg.from_admin ?
                    "bg-brand-navy border border-brand-cyan/20 rounded-tl-sm"
                  : "bg-brand-cyan/15 border border-brand-cyan/30 rounded-tr-sm"
                }`}
              >
                {msg.from_admin && (
      ...
      <form onSubmit={handleSend} className="flex gap-3">
        <textarea
          ...
          placeholder="Type your feedback or question... (Enter to send)"
        />
        <button
          type="submit"
              ...
              {milestones.map((m, i) => (
                <div key={m.id} className="flex gap-5 relative">
                  <div
                    ...
                    ${
                      m.completed ?
                        "bg-brand-cyan text-brand-darker font-bold"
                      : "bg-brand-navy border-2 border-white/10 text-white/20"
                    }`}
                  >
                    {m.completed ? "✓" : i + 1}
                  </div>
